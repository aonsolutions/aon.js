...
*/
	...
	module.exports.set = function(pool, aagreement, cb) {
		...
		database.query(pool,
			...
			function(error, results, fields){
				...
				ePaymentsConcept(function(sql,next){
					//logs sql statement
					console.log(sql);
					// executes sql statement
					database.query(sql, function callback (error, results, fields){
		...
		return function (pool, aagreement){
			...
			database.query(pool,
				...
				function(error, results, fields){
					...
					var conceptMapZero = {};
					for(var i=0; i<results.length; i++){
						conceptMapZero[results[i].code] = results[i].description;
					}
	...
	var setPayments = function(next){
		return function (pool, aagreement, payConceptMap){
