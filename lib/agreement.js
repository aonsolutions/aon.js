...
function _insertPaymentExtra(agreement, callback){
	if (agreement.extras){
		...
		for ( var i = 0; i < agreement.extras.length; i++){
			if(agreement.extras[i].agreePayment.code){
				replace = replace.insert(
		    	master.agreementPayment.agreement.value(agreement.id)
	        ,master.agreementPayment.domain.value(agreement.domain)
					,master.agreementPayment.paymentConcept.value(agreement.extras[i].agreePayment.__payConcept.id)
	        ...
	        ,master.agreementPayment.startDate.value(agreement.extras[i].agreePayment.startDate)
	        ,master.agreementPayment.descriptionDecorable.value(aagreement.extras[i].agreePayment.descriptionDecorable)
		...
		callback(replace.toQuery(), function () {  _insertExtra(agreement, callback) } );
	}else{
	...
	if(agreement.payments){
		...
		for ( var i = 0; i < agreement.payments.length; i++){
			if(agreement.payments[i].code){
		    	...
		    	master.agreementPayment.agreement.value(agreement.id)
	        ,master.agreementPayment.domain.value(agreement.domain)
					,master.agreementPayment.paymentConcept.value(agreement.payments[i].__payConcept.id)
...
function _insertPaymentsConcept(agreement, callback){
	//PARCHE
	var oldDescriptionDecorableProperty = master.paymentConcept.descriptionDecorable.property;
	var oldeIrpfExpression = master.paymentConcept.irpfExpression.property;
	var oldQuoteExpression = master.paymentConcept.quoteExpression.property;
	master.paymentConcept.descriptionDecorable.property = master.paymentConcept.descriptionDecorable.name;
	master.paymentConcept.irpfExpression.property = master.paymentConcept.irpfExpression.name;
	master.paymentConcept.quoteExpression.property = master.paymentConcept.quoteExpression.name;

	if(agreement.payments){
		for ( var i = 0; i < agreement.payments.length; i++){
			var insert = master.paymentConcept.insert(
				master.paymentConcept.id.value(agreement.payments[i].__payConcept.id)
        ...
        ,master.paymentConcept.quoteExpression.value(agreement.payments[i].quoteExpression)
				...
				code : agreement.payments[i].code,
				type : agreement.payments[i].type,
				expression : agreement.payments[i].expression,
				description : agreement.payments[i].description,
				description_decorable : agreement.payments[i].descriptionDecorable,
				irpf_expression : agreement.payments[i].irpfExpression,
				quote_expression : agreement.payments[i].quoteExpression
			}).toQuery();
			callback(insert, function () { } );
		}
	}
	...
	if(agreement.extras){
		for ( var i = 0; i < agreement.extras.length; i++){
			var insert = master.paymentConcept.insert(
				master.paymentConcept.id.value(agreement.extras[i].agreePayment.__payConcept.id)
				,master.paymentConcept.domain.value(agreement.domain)
				,master.paymentConcept.code.value(agreement.extras[i].agreePayment.code)
				,master.paymentConcept.type.value(agreement.extras[i].agreePayment.type)
				,master.paymentConcept.expression.value(agreement.extras[i].agreePayment.expression)
				,master.paymentConcept.description.value(agreement.extras[i].agreePayment.description)
				,master.paymentConcept.descriptionDecorable.value(agreement.extras[i].agreePayment.descriptionDecorable)
				,master.paymentConcept.irpfExpression.value(agreement.extras[i].agreePayment.irpfExpression)
				,master.paymentConcept.quoteExpression.value(agreement.extras[i].agreePayment.quoteExpression)
			).onDuplicate({
				code : agreement.extras[i].agreePayment.code,
				type : agreement.extras[i].agreePayment.type,
				expression : agreement.extras[i].agreePayment.expression,
				description : agreement.extras[i].agreePayment.description,
				description_decorable : agreement.extras[i].agreePayment.descriptionDecorable,
				irpf_expression : agreement.extras[i].agreePayment.irpfExpression,
				quote_expression : agreement.extras[i].agreePayment.quoteExpression
			}).toQuery();
			callback(insert, function () { } );
		}
	}
	...
	//DEJARLO COMO ESTABA
	master.paymentConcept.descriptionDecorable.property = oldDescriptionDecorableProperty;
	master.paymentConcept.irpfExpression.property = oldeIrpfExpression;
	master.paymentConcept.quoteExpression.property = oldQuoteExpression;
...
function _setUpPaymentsConcept(agreement, callback) {
		...
		function () {
			...
			if(agreement.payments){
				for ( var i = 0; i < agreement.payments.length; i++){
					if(agreement.payments[i].__payConcept.id == null)
						agreement.payments[i].__payConcept.id = master.paymentConcept.literal('( @MAX_PAYMENT_CONCEPT_ID + '+(i+1)+')');
					cont++;
				}
			}
			if(agreement.extras){
				for ( var j= 0; j < agreement.extras.length; j++){
					if(agreement.extras[j].agreePayment.__payConcept.id == null)
						agreement.extras[j].agreePayment.__payConcept.id = master.paymentConcept.literal('( @MAX_PAYMENT_CONCEPT_ID + '+(cont+1)+')');
					cont++;
				}
			}
