...
*/
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement){
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (payment_rec.__concept == undefined){ //!payment_rec.__concept
					console.log(payment_rec.code + "----------------------- LLEGA HASTA AQUI -------------------------------------");
					...
					insertPayConcept.insert(values);
					console.log(values);
				}
			}
			setConcept += insertPayConcept.text;
			insertPayConcept.toQuery();
			callback( {sql:setConcept, nestTables: true, values:insertPayConcept.values} , next );
		}
	}
	...
	var setPaymentsExtras = function(next){
		return function (callback, aagreement){
			...
			var query = setPay + insertPayExtra.text;
			insertPayExtra.toQuery();
			callback( {sql:query, nestTables: true, values:insertPayExtra.values} , next );
		}
	}
		...
		return function (callback, aagreement){
			...
			for(var i = 0; i < aagreement.levelsCategory.length; i++){
				...
				if (null == level_id){ //Si es un nivel nuevo se crea
					var values = {};
					values[level_rec.id.name] = "@LEVEL_ID"+i;
					values[level_rec.domain.name] = domain;
					values[level_rec.agreement.name] = agreement_id;
					values[level_rec.description.name] = level_description;
				...
				var query = setLevel + updateLevel + insertLevel.text;
				insertLevel.toQuery();
				callback( {sql:query, nestTables: true, values:insertLevel.values} , next );
			}
		}
	}
	...
	var seAgreementLevelCategory = function(next){
		return function (callback, aagreement){
			...
			var query = updateLevelCategory + insertLevelCategory;
			query.toQuery();
			callback( {sql:query.text, nestTables: true, values:query.values} , next );
		}
	}
		...
		return function (callback, aagreement){
			...
			for(var i = 0; i < aagreement.periods.length; i++){
				...
				for(var j = 0; j < period.levels.length; j++){
					...
					for(var data in level_rec.datas){
						var data_name = data;
						var data_expression = level_rec.datas[data];
			...
			var query = queryAgreeData + queryLevelData;
			query.toQuery();
			callback( {sql:query.text, nestTables: true, values:query.values} , next );
		}
	}
