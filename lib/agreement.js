...
*/
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			var query = "SET @CONCEPT_ID = (SELECT MAX(id) FROM payment_concept)";
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (conceptMapZero[payment_rec.code] == undefined){
					...
					var values = {};
					values[payment_concept.id.name] = "@CONCEPT_ID"+i;
					...
					payment_rec.__conceptId = "@CONCEPT_ID"+i;

					insertPayConcept = insertPayConcept.insert(values);
				}
			}
			insertPayConcept.toQuery();
			callback( {sql:insertPayConcept.text, nestTables: true, values:insertPayConcept.values} , next );
		}
	}
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (conceptMapZero[payment_rec.code] != undefined){
					...
					*/
					...
					var payConceptId = payment_rec.__conceptId;
					var startDate = new Date(0);
	...
	var setPaymentsExtras = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			var query = "SET @PAY_ID = (SELECT MAX(id) FROM agreement_payment)";
			...
			for(var i = 0; i < aagreement.extras.length; i++){
				...
				var values = {};
				values[payment.id.name] = "@PAY_ID"+i;
				values[payment.domain.name] = domain;
				values[payment.agreement.name] = agreement_id;
				...
				agreePay.__payId = "@PAY_ID"+i;

				insertPayExtra.insert(values);
			}

			insertPayExtra.toQuery();
			callback( {sql:insertPayExtra.text, nestTables: true, values:insertPayExtra.values} , next );
		}
	}
	...
	var setAgreementLevel = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			var query = "SET @LEVEL_ID = (SELECT MAX(id) FROM agreement_level)";
			...
			for(var i = 0; i < aagreement.levelsCategory.length; i++){
				...
				if (null == level_id){ //Si es un nivel nuevo se crea
					var values = {};
					values[level.id.name] = "@LEVEL_ID"+i;
				...
				insertLevel.toQuery();
				callback( {sql:insertLevel.text, nestTables: true, values:insertLevel.values} , next );
