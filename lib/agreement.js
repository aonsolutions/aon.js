...
*/
	...
	module.exports.set = function(pool, aagreement, cb) {
		...
		database.query(pool,
			...
			function(error, results, fields){
				...
				var agreementLevelCategory = setAgreementLevelCategory(datas);
				...
				var agreementTable = setAgreement(ePaymentsConcept);


				agreementTable(function callback (sqls,next){
					// Logs sql statement
					console.log(sqls);
					next(callback, aagreement);
	...
	var setAgreement = function(next){
		return function (callback, aagreement){
			...
			if(aagreement.id == null){
				...
				sqlsText.push("SET @AGREEMENT_ID = (SELECT MAX(id) FROM agreement);");
				sqltValues.push([]);

				var values = {};
				values[agreement.id.name] = "@AGREEMENT_ID";
				...
				sqlsText.push(insert.text);
				sqltValues.push(insert.values);
				...
				sqlsText = query.text;
				sqltValues = query.values;
			}

			if(aagreement.id == null){
				aagreement.id = "@AGREEMENT_ID";
			}

			callback( {sql:sqlsText, nestTables: true, values:sqltValues} , next );
		}
	}
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement){
			...
			// Get MAX Id fron payment_concept table
			insertText.push("SET @CONCEPT_ID = (SELECT MAX(id) FROM payment_concept);");
			insertValues.push([]);
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (payment_rec.__concept == undefined){
					...
					payment_rec.__concept = {};
					payment_rec.__concept["id"] = "@CONCEPT_ID+"+i;
			...
			insertText.push(insert.text);
			insertValues.push(insert.values);

			callback( {sql:insertText, nestTables: true, values:insertValues} , next );
		}
	}
	...
	var setPayments = function(next){
		return function (callback, aagreement){
			...
			var insert = insertPayments.toQuery();
			callback( {sql:insert.text, nestTables: true, values:insert.values} , next );
		}
	}
	...
	var setPaymentsExtras = function(next){
		return function (callback, aagreement){
			...
			var setPay = "SET @PAY_ID = (SELECT MAX(id) FROM agreement_payment);";
			...
			var insert = insertPayExtra.toQuery();
			setPay += insert.text;
			callback( {sql:setPay, nestTables: true, values:insert.values} , next );
		}
	}
	...
	var setExtras = function(next){
		return function (callback, aagreement){
			...
			var insert = insertExtra.toQuery();
			callback( {sql:insert.text, nestTables: true, values:insert.values} , next );
		}
	}
	...
	var setAgreementLevel = function(next){
		return function (callback, aagreement){
			...
			var setLevel = "SET @LEVEL_ID = (SELECT MAX(id) FROM agreement_level);";
			...
			//Insert and updates values
			var iu_values;
			var insert = insertLevel.toQuery ? insertLevel.toQuery() : "";
			if(insert != ""){
				setLevel += insert.text;
				iu_values += insert.values;
			}

			var update = updateLevel.toQuery ? updateLevel.toQuery() : "";
			if(update != ""){
				setLevel += update.text;
				iu_values = iu_values == undefined ? iu_values = update.values : iu_values += update.values ;
			}

			callback( {sql:setLevel, nestTables: true, values:iu_values} , next );
		}
	}

	var setAgreementLevelCategory = function(next){
		return function (callback, aagreement){
			...
			if(update != ""){
				...
				setLevelCat = setLevelCat == undefined ? setLevelCat = update.text : setLevelCat += update.text ;
				iu_values = iu_values == undefined ? iu_values = update.values : iu_values += update.values ;
			}

			callback( {sql:setLevelCat, nestTables: true, values:iu_values} , next );
		}
	}
	...
	var setDatas = function(next){
		return function (callback, aagreement){
			...
			if(insertAgreeData != ""){
				set_data = insertAgreeData.text;
				iu_values = insertAgreeData.values;
			}

			var insertLvlData = queryLevelData.toQuery ? queryLevelData.toQuery() : "";
			if(insertLvlData != ""){
				set_data = set_data == undefined ? set_data = insertLvlData.text : set_data += insertLvlData.text ;
				iu_values = iu_values == undefined ? iu_values = insertLvlData.values : iu_values += insertLvlData.values ;
			}

			callback( {sql:set_data, nestTables: true, values:iu_values} , next );
		}
	}
