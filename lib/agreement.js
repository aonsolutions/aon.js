...
*/
	...
	module.exports.set = function(pool, aagreement, cb) {
		...
		if(aagreement.id == null){
			...
			var setAgree = "SET @AGREEMENT_ID = (SELECT MAX(id) FROM agreement);";
			var values = {};
			...
			setAgree += insert.text;

			q_text = setAgree;
			q_values = insert.values;
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement){
			...
			// Get MAX Id fron payment_concept table
			var setConcept = "SET @CONCEPT_ID = (SELECT MAX(id) FROM payment_concept);";
			...
			var insert = insertPayConcept.toQuery();
			setConcept += insert.text;
			callback( {sql:setConcept, nestTables: true, values:insert.values} , next );
		}
	}
