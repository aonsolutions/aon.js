...
function _setAgreeData(agreement, callback){
	if (agreement.periods){
		...
		for ( var i = 0; i < agreement.periods.length; i++){
			for ( var j = 0; j < agreement.periods[i].levels.length; j++){
				if(agreement.periods[i].levels[j].id == 0){ //If id exist == AgreementData
...
function _setLevelData(agreement, callback){
	if (agreement.periods){
		...
		callback(replace.toQuery(), function () { _setAgreeData( agreement, callback ) } );
	}
}
...
function _setCategories(agreement, callback){
	if (agreement.levels){
		...
		callback(replace.toQuery(), function () { _setLevelData( agreement, callback ) } );
	}else{
		_setLevelData( agreement, callback );
	}
}
	...
	if (agreement.levels){
		...
		for ( var i = 0; i < agreement.levels.length; i++){
	        ...
	        ,master.agreementLevel.domain.value(agreement.domain)
				...
				var update = master.agreementLevel.update(
					{
						"description" : agreement.levels[i].description
					}
				).where(master.agreementLevel.id.equals(agreement.levels[i].id))
				.toQuery();
...
function _eraseLevelDatas(agreement, callback){
	var replace = master.agreementLevelData
			.delete()
...
function _eraseDatas(agreement, callback){
	var replace = master.agreementData
			.delete()
	...
	callback(replace, function() { _eraseLevelDatas(agreement, callback) });
}

function _erasePaymentConcept(agreement, callback){
	var replace = master.paymentConcept
			.delete()
	...
	callback(replace, function() { _eraseDatas(agreement, callback) });
}

function _erasePayments(agreement, callback){
	var replace = master.agreementPayment
			.delete()
			.from(master.agreementPayment)
	...
	callback(replace, function() { _erasePaymentConcept(agreement, callback) });
}

function _eraseExtras(agreement, callback){
	var replace = master.agreementExtra
			.delete()
	...
	callback(replace, function() { _erasePayments(agreement, callback) });
}
...
function _updateAgreement(agreement, callback){
	var update = master.agreement.update(
		{
	...
	callback(update, function() { _eraseExtras(agreement, callback) });
}
...
function _setUpAgreement(agreement, callback) {
		if(agreement.id == null){
			...
			_updateAgreement(agreement, callback);
		}
}
