...
*/
			...
			function(error, results, fields){
				...
				database.query(pool,
					...
					function(error, results, fields){
						...
						var conceptMapZero = {};
						for(var i=0; i<results.length; i++){
							conceptMapZero[results[i].code] = results[i].code;
							conceptMapZero[results[i].code]["values"] = {};
							conceptMapZero[results[i].code]["values"]["id"] = results[i].id;
							conceptMapZero[results[i].code]["values"]["code"] = results[i].code;
							conceptMapZero[results[i].code]["values"]["expression"] = results[i].expression;
							conceptMapZero[results[i].code]["values"]["description"] = results[i].description;
							conceptMapZero[results[i].code]["values"]["type"] = results[i].type;
							conceptMapZero[results[i].code]["values"]["descriptionDecorable"] = results[i].descriptionDecorable;
							conceptMapZero[results[i].code]["values"]["irpfExpression"] = results[i].irpfExpression;
							conceptMapZero[results[i].code]["values"]["quoteExpression"] = results[i].quoteExpression;
						}
						...
						var paymentConcept = setPaymentsConcept(payments);
						var eLevelData = eraseLevelData(paymentConcept);
						...
						ePaymentsConcept(function callback (sql,next){
							//logs sql statement
							console.log(sql);
							// executes sql statement
							database.query(sql, function (error, results, fields){
								next(callback, agreement);
							})
						},
						aagreement, conceptMapZero);
	...
	var erasePaymentsConcept = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			callback( {sql:query.text, nestTables: true, values:query.values} , next );
		}
	}
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement, conceptMapZero){

			//Obtenemos el dominio el agreement ID
			var domain = aagreement.domain;
			var agreement_id = aagreement.id;
			var insertPayConcept = payment_concept.insert();
			var payConceptMap = {};

			var query = "SELECT MAX(id) FROM payment_concept";
			var newId = null; //¿COMO EJECUTO ESTA SELECT Y COJO EL VALOR DEVUELTO?

			for(var i = 0; i<aagreement.payments.length; i++){
				var payment_rec = aagreement.payments[i];
				if (conceptMapZero[payment_rec.code] == undefined){
					...
					var values = {};
					values[payment_concept.id.name] = newId+i;
					...
					payConceptMap[payConceptCode] = newId+i;

					insertPayConcept = insertPayConcept.insert(values);
				}
			}
			insertPayConcept.toQuery();
			callback( {sql:insertPayConcept.text, nestTables: true, values:insertPayConcept.values} , next, payConceptMap );
			//¿ESTA BIEN PASADO ESE NUEVO PARAMETRO? ES PARA SIMULAR EL COMPORTAMIENTO DEL CODIGO DE ABAJO

			// insertPayConcept.toQuery();
			//
			// database.insert(pool,
			// 	insertPayConcept.text,
			// 	insertPayConcept.values,
			// 	function(error, results, fields){
			// 		if (error) throw error;
			// 		for(var i=0; i<results.length; i++){
			// 			payConceptMap[results[i].code] = results[i].insertId;
			// 		}
			// 		next(pool, aagreement, payConceptMap);
			// 	}
			// );
		}
	}
	...
	var setPayments = function(next){
		return function (callback, aagreement, conceptMapZero, payConceptMap){

			//Obtenemos el dominio el agreement ID
			var domain = aagreement.domain;
			var agreement_id = aagreement.id;
			var insertPayments = payment.insert();

			for(var i = 0; i<aagreement.payments.length; i++){
				var payment_rec = aagreement.payments[i];
				if (payConceptMap[payment_rec.code] != undefined){
					var payConceptId = payConceptMap[payment_rec.code];
					...
					insertPayments.insert(values);
				}else if (conceptMapZero[payment_rec.code] != undefined){
					vvar valuesMapZero = conceptMapZero[payment_rec.code]["values"];
					var payConceptId = valuesMapZero["id"];
					var payConceptCode = valuesMapZero["code"];
					var payConceptExpression = valuesMapZero["expression"];
					var payConceptDescription = valuesMapZero["description"];
					var payConceptType = valuesMapZero["type"];
					var payConceptDescriptionDecorable = valuesMapZero["descriptionDecorable"];
					var payConceptIrpfExpression = valuesMapZero["irpfExpression"];
					var payConceptQuoteExpression = valuesMapZero["quoteExpression"];
					var startDate = new Date(0);
			...
			insertPayments.toQuery();
			callback( {sql:insertPayments.text, nestTables: true, values:insertPayments.values} , next );
		}
	}
	...
	var setPaymentsExtras = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			var query = "SELECT MAX(id) FROM agreement_payment";
			var newId = null; //¿COMO EJECUTO ESTA SELECT Y COJO EL VALOR DEVUELTO?
			...
			for(var i = 0; i < aagreement.extras.length; i++){
				...
				var values = {};
				values[payment.id.name] = newId+i;
				values[payment.domain.name] = domain;
				values[payment.agreement.name] = agreement_id;
				...
				payMap[agreepay_description] = newId+i;

				insertPayExtra.insert(values);
			}

			insertPayExtra.toQuery();
			callback( {sql:insertPayExtra.text, nestTables: true, values:insertPayExtra.values} , next, payMap );
			//¿ESTA BIEN PASADO ESE NUEVO PARAMETRO? ES PARA SIMULAR EL COMPORTAMIENTO DEL CODIGO DE ABAJO
	...
	var setExtras = function(next){
		return function (callback, aagreement, conceptMapZero, payMap){
			...
			insertExtra.toQuery();
			callback( {sql:insertExtra.text, nestTables: true, values:insertExtra.values} , next );
		}
	}
	...
	var setAgreementLevel = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			var query = "SELECT MAX(id) FROM agreement_level";
			var newId = null; //¿COMO EJECUTO ESTA SELECT Y COJO EL VALOR DEVUELTO?
			...
			for(var i = 0; i < aagreement.levelsCategory.length; i++){
				...
				if (null == level_id){ //Si es un nivel nuevo se crea
					...
					levelsMap[level_description] = level_id;

					updateLevel.update(values).where(level.id.equals(level_id));
				}

				//¿COMO EJECUTO ESTAS DOS SIN QUE LA PRIMERA LLAME A LA SIGUIENTE FUNCION Y QUE ASI SE EJECUTEN LAS DOS?
				updateLevel.toQuery();
				callback( {sql:updateLevel.text, nestTables: true, values:updateLevel.values} , next );
				insertLevel.toQuery();
				callback( {sql:insertLevel.text, nestTables: true, values:insertLevel.values} , next, levelsMap );
				//¿ESTA BIEN PASADO ESE NUEVO PARAMETRO? ES PARA SIMULAR EL COMPORTAMIENTO DEL CODIGO DE ABAJO
	...
	var seAgreementLevelCategory = function(next){
		return function (callback, aagreement, conceptMapZero, levelsMap){
			...
			callback( {sql:updateLevelCategory.text, nestTables: true, values:updateLevelCategory.values} , next );
			insertLevelCategory.toQuery();
			callback( {sql:insertLevelCategory.text, nestTables: true, values:insertLevelCategory.values} , next );
		}
	}
	...
	var setDatas = function(next){
		return function (callback, aagreement, conceptMapZero){
			...
			callback( {sql:queryAgreeData.text, nestTables: true, values:queryAgreeData.values} , next );
			queryLevelData.toQuery();
			callback( {sql:queryLevelData.text, nestTables: true, values:queryLevelData.values} , next );
		}
	}
