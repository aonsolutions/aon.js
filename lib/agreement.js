...
function draftAgreementRecord(agreement, callback){
	var query = agreementTable
			.update({
				id : agreementTable.id.value(agreementTable.id*-1)
			})
			.from(agreementTable)
...
function draftLevels(agreement, callback){
	if(agreement.draft){
		var query = agreementLevelTable
				.update({
					id : agreementLevelTable.id.value(agreementLevelTable.id*-1),
					agreement : agreementLevelTable.agreement.value(agreementLevelTable.agreement*-1)
				})
...
function draftLevelDatas(agreement, callback){
	//PARCHE
	var oldAgreementLevelDataProperty = agreementLevelDataTable.agreementLevel.property;
	agreementLevelDataTable.agreementLevel.property = agreementLevelDataTable.agreementLevel.name;

	var query = agreementLevelDataTable
			.update({
				id : agreementLevelDataTable.id.value(agreementLevelDataTable.id*-1),
				agreement_level : agreementLevelDataTable.agreementLevel.value(agreementLevelDataTable.agreementLevel*-1)
			})
	...
	//DEJARLO COMO ESTABA
	agreementLevelDataTable.agreementLevel.property = oldAgreementLevelDataProperty;
...
function draftContractLevels(agreement, callback){
	//PARCHE
	var oldAgreementLevelCategoryProperty = contractTable.agreementLevelCategory.property;
	contractTable.agreementLevelCategory.property = contractTable.agreementLevelCategory.name;

	var query = contractTable
			.update({
				agreement_level_category : (contractTable.agreement_level_category*(-1))
			})
	...
	//DEJARLO COMO ESTABA
	contractTable.agreementLevelCategory.property = oldAgreementLevelCategoryProperty;
...
function draftLevelCategories(agreement, callback){
	//PARCHE
	var oldAgreementLevelProperty = agreementLevelCategoryTable.agreementLevel.property;
	agreementLevelCategoryTable.agreementLevel.property = agreementLevelCategoryTable.agreementLevel.name;

	var query = agreementLevelCategoryTable
			.update({
				id : agreementLevelCategoryTable.id.value(agreementLevelCategoryTable.id*-1),
				agreement_level : agreementLevelCategoryTable.agreementLevel.value(agreementLevelCategoryTable.agreementLevel*-1)
			})
			.where(agreementLevelCategoryTable.agreementLevel.in(
				agreementLevelTable.select(agreementLevelTable.id)
				.from(agreementLevelTable)
	...
	//DEJARLO COMO ESTABA
	agreementLevelCategoryTable.agreementLevel.property = oldAgreementLevelProperty;
...
function draftDatas(agreement, callback){
	var query = agreementDataTable
			.update({
				id : agreementDataTable.id.value(agreementDataTable.id*-1),
				agreement : agreementDataTable.agreement.value(agreementDataTable.agreement*-1)
			})
...
function draftPaymentConcept(agreement, callback){
	for(var i=0; i<agreement.payments.length; i++){
		...
		if(payment.__payConcept.id && payment.__payConcept.id != null){
			var query = paymentConceptTable
					.update({
						id : paymentConceptTable.id.value(paymentConceptTable.id*-1)
					})
	...
	draftDatas(agreement, callback);
}

function draftPayments(agreement, callback){
	//PARCHE
	var oldPaymentPaymentConceptProperty = agreementPaymentTable.paymentConcept.property;
	agreementPaymentTable.paymentConcept.property = agreementPaymentTable.paymentConcept.name;

	var query = agreementPaymentTable
			.update({
				id : agreementPaymentTable.id.value(agreementPaymentTable.id*-1),
				agreement : agreementPaymentTable.agreement.value(agreementPaymentTable.agreement*-1),
				payment_concept : agreementPaymentTable.paymentConcept.value(agreementPaymentTable.paymentConcept*-1)
			})
	...
	//DEJARLO COMO ESTABA
	agreementPaymentTable.paymentConcept.property = oldPaymentPaymentConceptProperty;
...
function draftExtras(agreement, callback){
	//PARCHE
	var oldAgreementExtraPaymentProperty = agreementExtraTable.agreementPayment.property;
	agreementExtraTable.agreementPayment.property = agreementExtraTable.agreementPayment.name;

	var query = agreementExtraTable
			.update({
				id : agreementExtraTable.id.value(agreementExtraTable.id*-1),
				agreement : agreementExtraTable.agreement.value(agreementExtraTable.agreement*-1),
				agreement_payment : agreementExtraTable.agreementPayment.value(agreementExtraTable.agreementPayment*-1)
			})
			.where(agreementExtraTable.agreement.equals(agreement.id))
			.toQuery();

	//DEJARLO COMO ESTABA
	agreementExtraTable.agreementPayment.property = oldAgreementExtraPaymentProperty;
