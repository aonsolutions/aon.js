...
*/
			...
			function(error, results, fields){
				...
				agreementTable(function callback (sqls,next){
					for (var i = 0; i < sqls.length; i++){
						console.log(sqls[i]);
						database.query(pool, sqls[i], function (error, results, fields){
	...
	var setPaymentsConcept = function(next){
		return function (callback, aagreement){
			...
			callback( [setconceptid, insert] , next );
	...
	var setPaymentsExtras = function(next){
		return function (callback, aagreement){
			...
			setpayid.values = [];
			// insertText.push("SET @PAY_ID = (SELECT MAX(id) FROM agreement_payment);");
			// insertValues.push([]);
	...
	var setAgreementLevel = function(next){
		return function (callback, aagreement){
			...
			setLevelId.values = [];
			// insertText.push("SET @LEVEL_ID = (SELECT MAX(id) FROM agreement_level);");
			// insertValues.push([]);
			...
			// insertText.push(update.text);
			// insertValues.push(update.values);

			callback( [setLevelId, insert, update] , next );
			// callback( {sql:insertText, nestTables: true, values:insertValues} , next );
		}
	}
	...
	var setDatas = function(next){
		return function (callback, aagreement){
			...
			// insertText.push(insertAgreeData.text);
			// insertValues.push(insertAgreeData.values);

			var insertLvlData = queryLevelData.toQuery ? queryLevelData.toQuery() : "";
			// insertText.push(insertLvlData.text);
			// insertValues.push(insertLvlData.values);

			callback( [insertAgreeData, insertLvlData] , next );
			// callback( {sql:insertText, nestTables: true, values:insertValues} , next );
		}
	}
