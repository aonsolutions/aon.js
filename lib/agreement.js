	...
	if (agreement.periods){
		...
		for ( var i = 0; i < agreement.periods.length; i++){
			for ( var j = 0; j < agreement.periods[i].levels.length; j++){
				var period = agreement.periods[i];
				var levelPeriod = agreement.periods[i].levels[j];
				var parsePeriodStartDate = parseDate(period.start_date);
				var parsePeriodEndDate = parseDate(period.end_date);
				...
				if(levelPeriod.id != 0){
			        ...
			        agreementLevelDataTable.domain.value(agreement.domain)
				    	...
				    	,agreementLevelDataTable.agreementLevel.value(levelPeriod.id)
							...
							,agreementLevelDataTable.startDate.value(period.start_date)
							,agreementLevelDataTable.endDate.value(period.end_date)
						);
					}
				}
			}
...
function insertCategories(agreement, callback){
	...
	//IF EXIST LEVELS
	if (agreement.levelsCategory){
		var insert = agreementLevelCategoryTable;
		for ( var i = 0; i < agreement.levelsCategory.length; i++){
			for ( var j = 0; j < agreement.levelsCategory[i].categories.length; j++){
				var level = agreement.levelsCategory[i];
				var categoryLevel = agreement.levelsCategory[i].categories[j];
				insert = insert.insert(
...
function insertExtra(agreement, callback){
	...
	if (agreement.extras){
        ...
        ,agreementExtraTable.startDate.value(extra.start_date)
        ,agreementExtraTable.endDate.value(extra.end_date)
        ,agreementExtraTable.issueDate.value(extra.issue_date)
...
function insertPaymentExtra(agreement, callback){
	...
	if (agreement.extras){
		...
		for ( var i = 0; i < agreement.extras.length; i++){
			var extra = agreement.extras[i];
			var parseStartDatePaymentExtraDate = parseDate(extra.agreePayment.start_date);
			var parseEndDatePaymentExtraDate = parseDate(extra.agreePayment.end_date);
        ...
        ,agreementPaymentTable.startDate.value(parseStartDatePaymentExtraDate)
				,agreementPaymentTable.endDate.value(parseEndDatePaymentExtraDate)
				,agreementPaymentTable.month.value(extra.agreePayment.month)
				,agreementPaymentTable.salaryType.value(extra.agreePayment.salary_type)
        ,agreementPaymentTable.descriptionDecorable.value(extra.agreePayment.description_decorable)
        ,agreementPaymentTable.irpfExpression.value(extra.agreePayment.irpf_expression)
        ,agreementPaymentTable.quoteExpression.value(extra.agreePayment.quote_expression)
	  	);
			// }
		}
...
function insertPayments(agreement, callback){
	...
	if(agreement.payments){
		...
		for ( var i = 0; i < agreement.payments.length; i++){
			...
			var parseStartDatePaymentDate = parseDate(payment.start_date);
			var parseEndDatePaymentDate = parseDate(payment.end_date);
        ...
        ,agreementPaymentTable.startDate.value(parseStartDatePaymentDate)
				,agreementPaymentTable.endDate.value(parseEndDatePaymentDate)
				,agreementPaymentTable.month.value(payment.month)
				,agreementPaymentTable.salaryType.value(payment.salary_type)
        ,agreementPaymentTable.descriptionDecorable.value(payment.description_decorable)
        ,agreementPaymentTable.irpfExpression.value(payment.irpf_expression)
        ,agreementPaymentTable.quoteExpression.value(payment.quote_expression)
	  	);
			// }
		}
	...
	if(agreement.payments){
		for ( var i = 0; i < agreement.payments.length; i++){
			...
			var insert = paymentConceptTable.insert(
				...
				,paymentConceptTable.domain.value(payment.__payConcept.domain)
				,paymentConceptTable.code.value(payment.__payConcept.code)
        ,paymentConceptTable.type.value(payment.__payConcept.type)
        ,paymentConceptTable.expression.value(payment.__payConcept.expression)
        ,paymentConceptTable.description.value(payment.__payConcept.description)
        ,paymentConceptTable.descriptionDecorable.value(payment.__payConcept.description_decorable)
        ,paymentConceptTable.irpfExpression.value(payment.__payConcept.irpf_expression)
        ,paymentConceptTable.quoteExpression.value(payment.__payConcept.quote_expression)
	  	).onDuplicate({
				code : payment.__payConcept.code,
				type : payment.__payConcept.type,
				expression : payment.__payConcept.expression,
				description : payment.__payConcept.description,
				description_decorable : payment.__payConcept.description_decorable,
				irpf_expression : payment.__payConcept.irpf_expression,
				quote_expression : payment.__payConcept.quote_expression
			}).toQuery();
			callback(insert, function () { } );
		}
	}
	...
	if(agreement.extras){
		for ( var i = 0; i < agreement.extras.length; i++){
			...
			var insert = paymentConceptTable.insert(
				...
				,paymentConceptTable.domain.value(extra.agreePayment.__payConcept.domain)
				,paymentConceptTable.code.value(extra.agreePayment.__payConcept.code)
				,paymentConceptTable.type.value(extra.agreePayment.__payConcept.type)
				,paymentConceptTable.expression.value(extra.agreePayment.__payConcept.expression)
				,paymentConceptTable.description.value(extra.agreePayment.__payConcept.description)
				,paymentConceptTable.descriptionDecorable.value(extra.agreePayment.__payConcept.description_decorable)
				,paymentConceptTable.irpfExpression.value(extra.agreePayment.__payConcept.irpf_expression)
				,paymentConceptTable.quoteExpression.value(extra.agreePayment.__payConcept.quote_expression)
			).onDuplicate({
				code :extra.agreePayment.__payConcept.code,
				type : extra.agreePayment.__payConcept.type,
				expression : extra.agreePayment.__payConcept.expression,
				description : extra.agreePayment.__payConcept.description,
				description_decorable : extra.agreePayment.__payConcept.description_decorable,
				irpf_expression : extra.agreePayment.__payConcept.irpf_expression,
				quote_expression : extra.agreePayment.__payConcept.quote_expression
			}).toQuery();
			callback(insert, function () { } );
		}
	}
