...
*/
		...
		return function (pool, aagreement){
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (payment_rec.code != null){ //Existe un payment_concept asociado
					...
					analizePaymentConcept(pool, domain, agreement_id, payment_rec, insertPayConcept);
			...
			database.insert(pool,
				...
				function(error, results, fields){
					if (error) throw error;
					for(var i=0; i<results.length; i++){
						payConceptMap[results[i].description] = results[i].insertId;
					}
	...
	analizePaymentConcept = function(pool, domain, agreement_id, payment_rec, insertPayConcept){
		var query = payment_concept
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				if (results.length == 0){
					insertEntryAgreementConcept(pool, domain, agreement_id, payment_rec, insertPayConcept);
				}
			}
		);
	}

	insertEntryAgreementConcept = function(pool, domain, agreement_id, payment_rec, insertPayConcept){
	...
	var setPayments = function(next){
		return function (pool, aagreement, payConceptMap){
			...
			for(var i = 0; i<aagreement.payments.length; i++){
				...
				if (payment_rec.code != null){ //Existe un payment_concept asociado
					//Insertaremos un payment, pero comparando sus valores con los del payment_concept asociado
					analizeAsociatedPaymentConcept(pool, domain, agreement_id, payment_rec, payConceptMap, insertPayments);
				}else{ //No existe payment_concept, por lo que hay que crearlo, y tambien el payment
					insertExistingAgreementConcept(pool, domain, agreement_id, payment_rec, insertPayments);
				}
			}
	...
	analizeAsociatedPaymentConcept = function(pool, domain, agreement_id, payment_rec, payConceptMap, insertPayments){
		var query = payment_concept
			.select(payment_concept.star())
			.from(payment_concept)
			.where(payment_concept.domain.equals(0))
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				var payConceptId = null;
				if (results.length == 0){
					payConceptId = payConceptMap[payment_rec.description];
					insertEntryAgreementNotExistingConcept(pool, domain, agreement_id, payConceptId, insertPayments);
				}else{
					if (undefined != results && results.length != 0){
						//GUARDAMOS LOS VALORES DEL PAYMENT_CONCEPT ASOCIADO
						 payConceptId = results[0].id;
						...
						*/
						...
						insertEntryAgreementExistingConcept(pool, domain, agreement_id, payConceptId,
							...
							payConceptDescriptionDecorable, payConceptIrpfExpression, payConceptQuoteExpression, insertPayments);
					}
				}
			}
		);
	...
	insertEntryAgreementNotExistingConcept = function(pool, domain, agreement_id, payConceptId, insertPayments){
		var startDate = new Date(0);
