		...
		return function (callback, aagreement){
			...
			for(var i = 0; i < aagreement.levelsCategory.length; i++){
				...
				if (null == level_id){ //Si es un nivel nuevo se crea
					...
					var values = {};
					values[level_rec.description.name] = level_description;
					console.log(level_rec.description.name);
					updateLevel = updateLevel.update(values).where(level.id.equals(level_id));
				}
				var insert = insertLevel.toQuery();
				var update = updateLevel.toQuery();
				setLevel += insert.text + update.text;
				var iu_values = insert.values + update.values; //Values de la insert y la update
