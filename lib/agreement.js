...
*/
	...
	insertEntryAgreementExistingConcept = function(pool, domain, agreement_id, payConceptId,
		...
		database.query(pool,
			...
			query.values,
			function(error, results, fields){
				console.log("----------- LLEGAMOS HASTA AQUI PAYMENTS CONCEPT -----------");
				if (error) throw error;
			}
		);
	}
	...
	insertEntryAgreementConcept = function(pool, domain, agreement_id, payment){
		...
		values[payment_concept.agreement.name] = agreement_id;
			...
			database.query(pool,
				...
				function(error, results, fields){
					...
					for(var i = 0; i < aagreement.extras.length; i++){
						var extra = aagreement.extras[i];
						var agreePay = extra.agreePayment;
						...
						insertEntryAgreementPayment(pool, domain, extra, agreement_id, agreepay_type,
	...
	insertEntryAgreementPayment = function(pool, domain, extra, agreement_id, agreepay_type, agreepay_expression,
	...
	insertEntryAgreementExtra = function(pool, domain, agreement_id, agree_id, extra_startDate,
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				console.log("----------- LLEGAMOS HASTA AQUI EXTRAS 2 -----------");
			}
		);
	}
	...
	var _setAgreementLevelAndCategory = function(next){
		return function (pool, aagreement){
			...
			database.query(pool,
				...
				function(error, results, fields){
					if (error) throw error;
					console.log("----------- LLEGAMOS HASTA AQUI LEVEL CATEGORY -----------");
					//Obtenemos el dominio el agreement ID
					var domain = results[0].domain;
					var agreement_id = aagreement.id;
	...
	insertEntryAgreementLevel = function(pool, domain, agreement_id, level_description){
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				console.log("----------- LLEGAMOS HASTA AQUI LEVEL -----------");
	...
	insertEntryAgreementLevelCategory = function(pool, domain, level_id, category_description){
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				console.log("----------- LLEGAMOS HASTA AQUI LEVEL CATEGORY 2 -----------");
			}
		);
	}
	...
	insertEntryAgreementData = function(pool, domain, data_name, agreement_id, data_expression,
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				console.log("----------- LLEGAMOS HASTA AQUI AGREEMENT DATA -----------");
			}
	 );
	}
	...
	insertEntryLevelData = function(pool, domain, data_name, level_id, data_expression,
		...
		database.query(pool,
			...
			function(error, results, fields){
				if (error) throw error;
				console.log("----------- LLEGAMOS HASTA AQUI LEVEL DATA -----------");
			}
