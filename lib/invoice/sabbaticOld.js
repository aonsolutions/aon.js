...
// BUSCAR DATA_rESPONSE POR TOKEN!
...
function updateDataResponse(pool, drId, invoiceId){
	...
	dataResponse.updateDataResponse(pool, data, function(error, result1){

	});

	f.module = "data";
	f.moduleId = drId;
	file.getFile(pool, filter, function(error, result2){
		var obj = new Object();
		obj.domain = result2.domain;
		obj.invoice = invoiceId;
		obj.mimeType = result2.mimeType;
		obj.description = result2.description;
		obj.data = result2.data;
		obj.type = 0;
		obj.attachDate = new Date();
		obj.driveId = result2.driveId;

		file.insertFile(pool, obj, function(error, result3){
