...
module.exports.standardPayroll = function(payroll, stream){
  ...
  var newAccrual = function(){
    ...
    salary_perceptions = getAccrual("1. Percepciones salariales");
    ...
    pdf
      ...
      for(var i=0; i<salary_perceptions.types.length; i++){
        salary_perception_type = salary_perceptions.types[i];
        pdf
          .text(salary_perception_type.type_expression , accrualX + paddingLeft2,  accrualY = accrualY + paddingTopTT)
          .text(formatAmount(salary_perception_type.type_value) , accrualX + paddingLeftValue + numberOffset(formatAmount(salary_perception_type.type_value)) , accrualY)
          .lineWidth(0.1)
          .moveTo(accrualX + paddingLeft2, accrualY + paddingTopLine).lineTo(accrualX + 376, accrualY + paddingTopLine).stroke();
      }

    salary_complements = getAccrual("Complementos salariales");
    pdf
      .text('Complementos salariales' , accrualX + paddingLeft2, accrualY = accrualY + paddingTopTL);

    if(salary_complements == undefined){
      pdf
    ...
    extra_hours = getAccrual("Horas extraordinarias");
    pdf
    ...
    extra_perks = getAccrual("4. Pagas extraordinarias prorrateo");
    pdf
      .text('Gratificaciones extraordinarias' , accrualX + paddingLeft2, accrualY = accrualY + paddingTopTL);

    if(extra_perks == undefined){
      pdf
        .moveTo(accrualX + paddingLeft2, accrualY + paddingTopLine).lineTo(accrualX + 376, accrualY + paddingTopLine).stroke();
    }else{
      total_perks = 0;
      for(j = 0; j < extra_perks.types.length; j++){
        total_perks += extra_perks.types[j].type_value;
      }
      pdf
        .text(formatAmount(total_perks) , accrualX + paddingLeftValue + numberOffset(formatAmount(total_perks)), accrualY)
        .moveTo(accrualX + paddingLeft2, accrualY + paddingTopLine).lineTo(accrualX + 376, accrualY + paddingTopLine).stroke();
    }

    spices_salary =  getAccrual("Salario en especie");
    pdf
      .text('Salario en especie' , accrualX + paddingLeft2, accrualY = accrualY + paddingTopTL);

    if(spices_salary == undefined){
      pdf
        .moveTo(accrualX + paddingLeft2, accrualY + paddingTopLine).lineTo(accrualX + 376, accrualY + paddingTopLine).stroke();
    }else{
      pdf
        .text(formatAmount(spices_salary.type_expression) , accrualX + paddingLeftValue + numberOffset(formatAmount(spices_salary.type_expression)), accrualY)
        .moveTo(accrualX + paddingLeft2, accrualY + paddingTopLine).lineTo(accrualX + 376, accrualY + paddingTopLine).stroke();
    }
