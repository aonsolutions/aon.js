...
function itemFilter(f){
	...
	if(f.detail) filter = filter ? filter.and(item.detail.equals(f.detail)) : item.detail.equals(f.detail);
	if(f.detail2) filter = filter ? filter.and(item.detail2.equals(f.detail2)) : item.detail2.equals(f.detail2);
	if(f.detail3) filter = filter ? filter.and(item.detail3.equals(f.detail3)) : item.detail3.equals(f.detail3);
	return filter;
}
