...
module.exports.get = function(pool, filter, callback) {
	...
	DATABASE.query(pool,
		...
		( error, results, fields ) => {
			settlements = results.map((result) => {
				return {
					_id: result.id,
		...
		var getDeductions = function () {
			...
			query = deduction
				...
				.from(deduction)
				.where(deduction.salary.in(settlements.map((s)=> { return s._id; })))
		...
		var getPayments = function () {
			...
			query = payment
				.select(payment.star())
				.from(payment)
				.where(payment.salary.in(settlements.map((s)=> { return s._id; })))
		...
		var getData = function () {
			...
			query = data
				.select(data.star())
				.from(data)
				.where(data.salary.in(settlements.map((s)=> { return s._id; })))
				.order(data.salary)
				.toQuery();
		...
		var getEmployeeAdress = function () {
			...
			query = salary
				...
				.where(salary.id.in(settlements.map((s)=> { return s._id; })))
				.order(salary.id)
				.toQuery();
...
module.exports.a3Letter = function (settlement, stream) {
	...
	var payments = settlement.payments.filter( ( p ) => p.amount > 0.00 )
	...
	var deductions = settlement.deductions.filter( ( p ) => p.amount > 0.00 )
